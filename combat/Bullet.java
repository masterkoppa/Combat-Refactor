...
public class Bullet extends Sprite {
	...
	public void conflict(Sprite enemy) {
		...
		// depends on the direction i'm heading and the direction of the barrier
			...
			if ((dir == DirectionalImage.North)
				...
				Point[] bar = this.getBox(enemy);
				Point barUpperLeft = bar[0];
				Point barLowerRight = bar[1];

				// and we need my position
				Point[] bullet = this.getBox(this);
				Point bulUpperLeft = bullet[0];
				Point bulLowerRight = bullet[1];
				...
				bounce(dir, barUpperLeft, barLowerRight, bulUpperLeft,
						bulLowerRight);
	...
	/**
	 * Create points for each of the bounds of the selected sprite
	 * 
	 * @param object
	 * @return
	 */
	private Point[] getBox(Sprite object) {

		Rectangle barrierBox = object.getBoundingBox();
	...
	private void bounce(int dir, Point barUpperLeft, Point barLowerRight,
			Point bulUpperLeft, Point bulLowerRight) {
	...
	public void moveForward() {
		if (onBoard) {
			...
			lastLowR = new Point(lastUpL.x + bulletBox.width, lastUpL.y
					+ bulletBox.height);
	...
	/*
	 * Right turn of the Bullet; will never be called in current implementation,
	 * as our bullets do not turn on command. here for completeness
	 */
	public void turnRight() {
	}

	/*
	 * left turn of the Bullet; will never be called in current implementation,
	 * as our bullets do not turn on command. here for completeness
	 */
	...
	/*
	 * The player makes and returns its next move this will never get called,
	 * since Players aren't ticked - their managers are. Here for consistancy
	 * 
